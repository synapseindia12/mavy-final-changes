...
myApp.config(function($routeProvider, $httpProvider, $facebookProvider, $locationProvider) {	
	...
	.run(function($rootScope, $window, $localStorage){
		(function(){
...
myApp.controller('ErrorCtrl', function($scope, $localStorage, $location, $route, $rootScope){
...
myApp.controller('signupCtrl', function($scope, $rootScope, $location, $cookieStore, $localStorage, $timeout, $window, $facebook, $modal, $route){
		...
		if($scope.sign.uname && $scope.sign.password){
			endpoints.mobileHandler.login($scope.sign.uname, $scope.sign.password, $scope.userSysteminfo, function(result){
				if(result.result.success){
					$('.username').removeClass('wrong_credential');
					$('.password').removeClass('wrong_credential');
					...
					$('.username').addClass('wrong_credential');
					$('.password').addClass('wrong_credential');
					$('.navigationBlock').hide();
					$scope.sign.uname = '';
					$scope.sign.password = '';
					$location.path('/');
			...
			if(result.status){
				...
				endpoints.mobileHandler.loginFb($scope.accessToken, $scope.userSystemInfo, function(response){
					if(response.result.success){
						...
						$localStorage.userDetails = {"userId": $scope.userId};
						$location.path('/dashboard');
						$scope.$apply();
					}
					else{
	...
	$scope.removeActive = function(){
		if($('.username').hasClass('wrong_credential') || $('.password').hasClass('wrong_credential')){
			$('.username').removeClass('wrong_credential');
			$('.password').removeClass('wrong_credential');
		}
	};
...
myApp.controller('indexCtrl', function($scope, $cookieStore, $rootScope, $localStorage, $location){
	...
	switch(x){
		case '/dashboard':
			$('.menu_right ul li a').removeClass('active');
			$('.menu_right ul li:nth-child(1) a').addClass('active');
			break;
		case '/assignment':
			$('.menu_right ul li a').removeClass('active');
			$('.menu_right ul li:nth-child(2) a').addClass('active');
			break;
		case '/assignments':
			$('.menu_right ul li a').removeClass('active');
			$('.menu_right ul li:nth-child(2) a').addClass('active');
		...
		case '/messages':
			$('.menu_right ul li a').removeClass('active');
			$('.menu_right ul li:nth-child(3) a').addClass('active');
			break;
		case '/forum':
			$('.menu_right ul li a').removeClass('active');
			$('.menu_right ul li:nth-child(4) a').addClass('active');
			break;
		default:
			$('.menu_right ul li a').removeClass('active');
			break;
	...
	$('.menu_right ul li a:first').addClass('active');
	/*********** ./Active Forum Threads **********/
});
...
myApp.controller('pollsCtrl', function($scope, $rootScope, $location, $localStorage, $cookieStore, $filter){
										...
										if($rootScope.deletedPolls.length > 0){
												...
												if(result.result.result.Entries[i].itemId == voteCounts.result.result[0].itemId){
													for(var j=0; j<result.result.result.Entries[i].options.categories.length; j++){
														if(voteCounts.result.result[0].values[j]){
															for(var q=0; q<result.result.result.Entries[i].options.categories.length; q++){
																if(voteCounts.result.result[0].values[j].value == result.result.result.Entries[i].options.categories[q].values){
																	voteCounts.result.result[0].values[j].count = Math.round((voteCounts.result.result[0].values[j].count /voteCounts.result.result[0].responseCount)*100);
																	$scope.pollforResults.push({'count': voteCounts.result.result[0].values[j].count, 'category': result.result.result.Entries[i].options.categories[q].description});
																}
															}
														}														
													}
											...
											if(result.result.result.Entries[i].itemId == voteCounts.result.result[0].itemId){
												for(var j=0; j<result.result.result.Entries[i].options.categories.length; j++){
													...
													// else{
														// for(var m=0; m<result.result.result.Entries[i].options.categories.length; m++){
															// for(var n=0; n<voteCounts.result.result[0].values.length; n++){
																// if(result.result.result.Entries[i].options.categories[m].values != voteCounts.result.result[0].values[n].value){
																	// $scope.pollforResults.push({'count': '0', 'category': result.result.result.Entries[i].options.categories[m].description});
																// }
															// }
														// }
													// }
	...
	$scope.submitPoll = function(pollDetails, pollvote){
		...
		if($('input[name=\"pollvote'+pollDetails.itemId+'\"]:checked').val()){
			$scope.value.push($('input[name=\"pollvote'+pollDetails.itemId+'\"]:checked').val());
		}
		
		if($('input[name="pollCheck"]').length > 0){
			for(var i=0; i<$('input[name="pollCheck"]').length; i++){
				if($('input[name="pollCheck"]')[i].closest('.checkBox-checked')){
					$scope.value.push($('input[name="pollCheck"]')[i].value);
				}
			}
		}
...
myApp.controller('navCtrl', function($scope, $cookieStore, $rootScope, $location, $localStorage, $timeout, $route){
	...
	$('.menu_right ul li a').on('click', function(){
		$('.menu_right ul li a').removeClass('active');
		$(this).addClass('active');
	});
	if($('.menu_right ul li a').hasClass('active')){		
		$('.menu_right ul li a').removeClass('active');		
		$('.menu_right ul li a:first').addClass('active');
	}
	else{		
...
myApp.controller('showassignmentCtrl', function($scope, $cookieStore, $rootScope, $location, $localStorage, $sce, $window, $route){
	...
	$scope.markCompleted = function(task) {
		...
		$scope.onsubmitPost = true;
		var reply = '';
		$scope.postBody = $('#reply').val();
		...
		if(uploadedMediaFileUrl){
			var bodyText;
			if(uploadedMediaFileType == 'image' || uploadedMediaFileType == 'Image'){
				if($scope.postBody)
					bodyText = $scope.postBody + " <img src=\""+uploadedMediaFileUrl+"\">";
				else
					bodyText = " <img src=\""+uploadedMediaFileUrl+"\">";
			}
			else{
				if($scope.postBody)
					bodyText = $scope.postBody + " [View:"+uploadedMediaFileUrl+":0:0]";
				else
					bodyText = " [View:"+uploadedMediaFileUrl+":0:0]";
			}
			uploadedMediaFileUrl = '';
			uploadedMediaFileType = '';
		}
		if(bodyText)
			reply=bodyText;
		else
			reply=$scope.postBody;
	...
	$scope.submitChildReply = function(Id, TaskId) {
		$scope.onsubmitChildPost = true;
		$scope.childReplyId = Id;
		$scope.TaskId = TaskId;
		var reply = '';
		$scope.postBody = $('#replyNewChild' + Id).val();
		if(uploadedMediaFileUrl){
			var bodyText;
			if(uploadedMediaFileType == 'image' || uploadedMediaFileType == 'Image'){
				if($scope.postBody)
		...
		if (input.files && input.files[0]) {
			...
			reader.onload = function (e) {
				...
				endpoints.mediaHandler.convertMedia(Apikey, media_base64, bucketName, projectId, sourceAppType, mediaType, $scope.data.userUpload, 0, function(result){
					if(result.result.success){
						$scope.uploadingMediaFile = false;
						$scope.showContent = true;
						$scope.disableSendButton = false;
						uploadedMediaFileUrl = result.result.result.URL;
						uploadedMediaFileType = mediaType;
						$scope.$apply();
					}
					else{
						alert('fail');
	...
	$scope.uploadMediaFile = function (input){
		$scope.uploadingMediaFile = true;
...
myApp.controller('forumCtrl', function($scope,$localStorage,$rootScope,$routeParams,$location, $route, $sce){	
	...
	uploadedMediaFileUrl = '',
	uploadedMediaFileType = '',
	uploadedChildMediaFileUrl = '',
	uploadedChildMediaFileType = '',
	uploadMediaType = '';
	endpoints.apiKey = $scope.apiKey;
	endpoints.mobileHandler = new MobileHandler();
	endpoints.mediaHandler = new MediaHandler();
	...
	$scope.submitPost = function(){
		...
		if(uploadedMediaUrl){
			...
			if(uploadMediaType == 'image' || uploadMediaType == 'Image'){
				if($scope.postBody)
					bodyText = $scope.postBody + " <img src=\""+uploadedMediaUrl+"\">";
				else
					bodyText = " <img src=\""+uploadedMediaUrl+"\">";
			}
			else{
				if($scope.postBody)
					bodyText = $scope.postBody + " [View:"+uploadedMediaUrl+":0:0]";
				else
					bodyText = " [View:"+uploadedMediaUrl+":0:0]";
			}
			uploadedMediaUrl = '';
			uploadMediaType = '';
		}
		if(bodyText)
			$scope.threadInfo=[{"name":"Subject","value":$scope.postTitle},{"name":"Body","value":bodyText}];
		else
	...
	$scope.saveThreadReply=function(replyText,id){
		$scope.onpostineReply = true;
		$scope.showUploadedContent = false;
		$scope.$apply();
		if(uploadedMediaFileUrl){
			if(uploadedMediaFileType == 'image' || uploadedMediaFileType == 'Image'){
				if($scope.replyText)
					replyText = $scope.replyText + "<img src=\""+uploadedMediaFileUrl+"\">";
				else
					replyText = "<img src=\""+uploadedMediaFileUrl+"\">";
			}
			else{
				if($scope.replyText)
					replyText = $scope.replyText + "[View:"+uploadedMediaFileUrl+":0:0]";
				else
					replyText = "[View:"+uploadedMediaFileUrl+":0:0]";
			}
		}
		if(replyText){
	...
	$scope.savechildThreadReply = function(threadId,parentId){
		var replyText = '';
		$scope.onpostingChildReply = true;
		$scope.showUploadedChildContent = false;
		$scope.$apply();
		if(uploadedChildMediaFileUrl){
			if(uploadedChildMediaFileType == 'image' || uploadedChildMediaFileType == 'Image'){
				if($('#displayReplyBox'+parentId+' textarea').val())
					replyText = $('#displayReplyBox'+parentId+' textarea').val() + "<img src=\""+uploadedChildMediaFileUrl+"\">";
				else
					replyText = "<img src=\""+uploadedChildMediaFileUrl+"\">";
			}
			else{
				if($('#displayReplyBox'+parentId+' textarea').val())
					replyText = $('#displayReplyBox'+parentId+' textarea').val() + "[View:"+uploadedChildMediaFileUrl+":0:0]";
				else
					replyText = "[View:"+uploadedChildMediaFileUrl+":0:0]";
			}
		}
		if($('#displayReplyBox'+parentId+' textarea').val()){
			if(!replyText)
				replyText = $('#displayReplyBox'+parentId+' textarea').val();
			endpoints.mobileHandler.saveReply($scope.apiKey,$scope.userId,threadId,parentId,replyText,function(response){
				if(response.result.success){
					uploadedChildMediaFileUrl = '';
					uploadedChildMediaFileType = '';
					$scope.onpostingChildReply = false;
					$scope.showUploadedChildContent = false;
		...
		if (input.files && input.files[0]) {
			...
			reader.onload = function (e) {
				...
				endpoints.mediaHandler.convertMedia(Apikey, media_base64, bucketName, projectId, sourceAppType, mediaType, $scope.data.userUpload, 0, function(result){
					if(result.result.success){
						$scope.uploadingMediaFile = false;
						$scope.showUploadedContent = true;
						$scope.disableSendButton = false;
						uploadedMediaFileUrl = result.result.result.URL;
						uploadedMediaFileType = mediaType;
						$scope.$apply();
	...
	$scope.uploadChildFile = function (input){
		$scope.uploadingChildMediaFile = true;
		$scope.disableChildSendButton = true;
		$scope.$apply();
		$scope.data = {};
		$scope.data.userUpload = '';
		$scope.fileName="";
		...
		if (input.files && input.files[0]){
			...
			reader.onload = function (e) {
				...
				endpoints.mediaHandler.convertMedia(Apikey, media_base64, bucketName, projectId, sourceAppType, mediaType, $scope.data.userUpload, 0, function(result){
					if(result.result.success){
						$scope.uploadingChildMediaFile = false;
						$scope.showUploadedChildContent = true;
						$scope.disableChildSendButton = false;
						uploadedChildMediaFileUrl = result.result.result.URL;
						uploadedChildMediaFileType = mediaType;
						$scope.$apply();
					}
					else{
						alert('fail');
...
myApp.controller('messagesCtrl', function($scope, $cookieStore, $rootScope, $localStorage, $location, $route, $timeout){
	if(!$localStorage.loginDetails){
		delete $localStorage.loggedIn;
		$location.path('/');
	}
...
myApp.controller('messageconversationCtrl', function($scope,$localStorage,$cookieStore,$routeParams,$rootScope, $timeout, $location){
	var projectId="";
	$rootScope.settings = false;
	$rootScope.rewards = false;
	$rootScope.badges = false;
...
myApp.controller('profileCtrl', function($scope, $localStorage,$cookieStore, $location, $rootScope, $route, $timeout){
	if(!$localStorage.loginDetails){
		delete $localStorage.loggedIn;
		$location.path('/');
	}
	...
	switch($location.path()){
		case '/rewards':
			$('.profile_listing ul li:nth-child(1) a').addClass('active');
			$('.menu_right ul li a').removeClass('active');
			break;
		case '/profile':
			$('.profile_listing ul li:nth-child(2) a').addClass('active');
			$('.menu_right ul li a').removeClass('active');
			break;
		case '/badges':
			$('.profile_listing ul li:nth-child(3) a').addClass('active');
			$('.menu_right ul li a').removeClass('active');
			break;
		default:
			$('.profile_listing ul li a').removeClass('active');
			break;
	}
	...
	$scope.resetPass = function(resetPassword) {
		$scope.disablePasswordChangeButton = true;
		...
		if($scope.resetPassword.vnpass == $scope.resetPassword.npass){
			endpoints.mobileHandler.changePassword($scope.apiKey, $scope.userId, $scope.currentPass, $scope.newPass, function(result){
				if(result.result.success){
					$scope.disablePasswordChangeButton = false;
					$scope.hidePasswordChangeButton = true;
					$scope.resetPassword.cpass = '';
					$scope.resetPassword.npass = '';
					$scope.resetPassword.vnpass = '';
					$timeout(function(){
						$scope.hidePasswordChangeButton = false;
						$rootScope.initialized = false;
						delete $localStorage.loggedIn;
						delete $localStorage.loginDetails;
						$location.path('/');
...
myApp.controller('notificationCtrl', function($scope, $localStorage, $location, $timeout){
	...
	$scope.updateNotification = function() {
		$scope.disableNotificationSaveButton = true;
		if($scope.emailCheck){
				...
				if(callback.result.success){
					if(callback.result.result.cellphone){
						...
						endpoints.mobileHandler.updateNotifications($scope.apiKey, $scope.userId, $scope.panelistId, $scope.preferences, function(result){
							$scope.hideNotificationSaveButton = true;
							$scope.disableNotificationSaveButton = false;
							$scope.$apply();
							$timeout(function(){
								$scope.hideNotificationSaveButton = false;
								$scope.disableNotificationSaveButton = false;
							}, 800);
						});
				}
...
myApp.controller('rewardsModalCtrl', function($scope, $modalInstance, rewards, apiKey, userId, panelistId, $rootScope){
	...
	$scope.ok = function () {
		...
		endpoints.mobileHandler.redeem(apiKey, userId, panelistId, function(result){
			if(result.result.success){
			...
			if(!$rootScope.initialized){
				...
				setTimeout(function(){
					// $("#pp-nav ul").on("DOMSubtreeModified", function() {
						...
						// }
					// });
					var whatToObserve = {childList: true, attributes: true, subtree: true, attributeFilter: ['class', 'style']};
				...
				// element.on('click', '.nav li.itemsNav a', function(){
					// var winW = $(window).width();
					// if(winW<768){
					   // $('.slide-bg-image .top-nav').slideUp(); 
					// }
					// $('.section').addClass('pp-easing');
					// var index = $(this).data('ind');
					// $('.section').removeClass('active');
					// $('.section').eq(index).addClass('active');
					// $('#pp-nav ul li a').removeClass('active');
					// $('#pp-nav ul li a').eq(index).addClass('active');
					// for(var i=$(this).data('ind')-1;i>=0;i--){
						// $('.section').eq(i).css({"transform": "translate3d(0px, -100%, 0px)"});
					// }					
				// });
				
				// element.on('click', '.footer .signup-links a',function(){
					// var winW = $(window).width();
					// if(winW<768){
					   // $('.slide-bg-image .top-nav').slideUp(); 
					// }
					...
					// $('#pp-nav ul li a').removeClass('active');
					// $('#pp-nav ul li a').eq(index).addClass('active');
					// $('.section').css({"transform": "translate3d(0px, 0%, 0px)"});
					// for(var i=$(this).data('ind')-1;i>=0;i--){
						// $('.section').eq(i).css({"transform": "translate3d(0px, -100%, 0px)"});
					// }
				// });
...
myApp.directive('uploadMedia', function(){
    return {
      restrict: 'A',
      link: function(scope, element){
        element.bind('click', function(e){
            angular.element(e.target).siblings('#mediaUpload').trigger('click');
        });
      }
    };
});
myApp.directive('customChecks', function(){
	return{
		restrict: 'A',
...
myApp.directive('preloader',['$location', '$timeout', function(location, timeout){
	return{
		restrict: 'A',
		link: function(scope, ele){
			var path = location.$$absUrl.split('#')[0] + 'images/login-bg.jpg';
			if(path){
				var img = new Image();
				img.onload = function() {
